(async function () {
  ...
  // --- Setup time entry comment caching ---
  ...
  function clearCache() {
    try {
      ...
      console.log("cache cleared:", loadCache())
    } catch (e) {
  ...
  function addCommentEntrySelect(issueInputField) {
    ...
    if (worklogCommentField && projectNumber) {
      console.log("[tempo-addCommentEntrySelect] project selected:", projectNumber);
      ...
      commentSelect.addEventListener("change", () => {
        ...
        worklogCommentField.dispatchEvent(new Event('input', { bubbles: true, cancelable: true }));
      });

      // time log get's updated
      document.getElementById("logTimeBtn").addEventListener("click", () => {
        if (issueInputField && worklogCommentField && worklogCommentField.value.trim()) {
          console.log("[tempo-addCommentEntrySelect] worklog comment recorded:", projectNumber, worklogCommentField.value.trim());
          addTimeEntryComment(worklogCommentField.value.trim(), projectNumber);
          setupModalObserver();
        }
      });
    }
  ...
  function setupModalObserver() {
    waitForElement('#form-issue-input', (elements) => {
      if (elements && elements.length) {
        ...
        callOnValueChanged('form-issue-input', () => {
          addCommentEntrySelect(document.getElementById('form-issue-input'));
        });
      }
    }, Number.MAX_VALUE);
  }
